...
public sealed class DisableApiKeyCommand : AsyncCommand<DisableApiKeyCommand.Settings>
{
    private readonly ILogger<DisableApiKeyCommand> _logger;
    private readonly IServiceProvider _serviceProvider;

    public DisableApiKeyCommand(ILogger<DisableApiKeyCommand> logger, IServiceProvider serviceProvider)
    {
        _logger = logger;
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        ...
        var response = await epochFlowApi.DisableApiKey(settings.Id);
        stopwatch.Stop();
